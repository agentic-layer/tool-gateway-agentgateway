...
var _ = Describe("ToolGateway Controller", func() {
	...
	Describe("Reconcile", func() {
		...
		It("should translate OTEL env vars to telemetry config and filter them from env", func() {
			toolGatewayClass := &agentruntimev1alpha1.ToolGatewayClass{
				ObjectMeta: metav1.ObjectMeta{
					Name: "test-class-otel",
				},
				Spec: agentruntimev1alpha1.ToolGatewayClassSpec{
					Controller: "runtime.agentic-layer.ai/tool-gateway-agentgateway-controller",
				},
			...
			toolGateway := &agentruntimev1alpha1.ToolGateway{
				ObjectMeta: metav1.ObjectMeta{
					Name:      "test-otel-gateway",
					Namespace: "default",
				},
				Spec: agentruntimev1alpha1.ToolGatewaySpec{
					ToolGatewayClassName: "test-class-otel",
					Env: []corev1.EnvVar{
						{Name: "OTEL_EXPORTER_OTLP_ENDPOINT", Value: "http://otel-collector:4318"},
						{Name: "OTEL_EXPORTER_OTLP_PROTOCOL", Value: "http/protobuf"},
						{Name: "OTEL_EXPORTER_OTLP_HEADERS", Value: "api-key=secret123"},
						{Name: "NORMAL_ENV_VAR", Value: "should-remain"},
					},
				},
			}
			Expect(k8sClient.Create(ctx, toolGateway)).To(Succeed())
			...
			// Check that AgentgatewayParameters was created with telemetry config
			params := &unstructured.Unstructured{}
			params.SetAPIVersion("agentgateway.dev/v1alpha1")
			params.SetKind("AgentgatewayParameters")
			Eventually(func() error {
			...
			// Verify telemetry config is present in rawConfig
			rawConfig, found, err := unstructured.NestedMap(params.Object, "spec", "rawConfig")
			Expect(err).NotTo(HaveOccurred())
			Expect(found).To(BeTrue())
			Expect(rawConfig).NotTo(BeNil())

			tracing, found, err := unstructured.NestedMap(rawConfig, "tracing")
			Expect(err).NotTo(HaveOccurred())
			Expect(found).To(BeTrue())

			endpoint, found, err := unstructured.NestedString(tracing, "otlpEndpoint")
			Expect(err).NotTo(HaveOccurred())
			Expect(found).To(BeTrue())
			Expect(endpoint).To(Equal("http://otel-collector:4318"))

			protocol, found, err := unstructured.NestedString(tracing, "otlpProtocol")
			Expect(err).NotTo(HaveOccurred())
			Expect(found).To(BeTrue())
			Expect(protocol).To(Equal("http"))

			headers, found, err := unstructured.NestedMap(tracing, "headers")
			Expect(err).NotTo(HaveOccurred())
			Expect(found).To(BeTrue())
			Expect(headers["api-key"]).To(Equal("secret123"))

			// Verify OTEL env vars are NOT in spec.env (filtered out)
			envVars, found, err := unstructured.NestedSlice(params.Object, "spec", "env")
			Expect(err).NotTo(HaveOccurred())
			Expect(found).To(BeTrue())
			Expect(envVars).To(HaveLen(1))

			// Check that only NORMAL_ENV_VAR remains
			env0 := envVars[0].(map[string]interface{})
			Expect(env0["name"]).To(Equal("NORMAL_ENV_VAR"))
			Expect(env0["value"]).To(Equal("should-remain"))
		})
	})
})
