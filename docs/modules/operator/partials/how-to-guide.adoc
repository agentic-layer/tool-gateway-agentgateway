...
* **https://agentgateway.dev/[agentgateway]**: Gateway implementation
* **xref:agent-runtime-operator:agent-runtime:how-to-guides/install.adoc[Agent Runtime Operator]**: Provides ToolGateway, ToolGatewayClass, and ToolServer CRDs
...
The operator can attach guardrails to a `ToolGateway` by deploying a `guardrail-adapter` instance per `Guard`. To enable this, the controller must be started with the `--guardrail-adapter-image=<image>:<tag>` flag. The shipped `install.yaml` already pins a known-compatible adapter version; if you build your own kustomize overlay, set the image via:
...
You also need a `GuardrailProvider` backing service (e.g. Presidio) reachable from the cluster. See xref:agent-runtime-operator:guardrails:how-to-guide.adoc[Create and Use Guardrails in Gateways] for the cross-gateway concepts and the xref:operator:reference.adoc#guardrails[Guardrails section in the reference] for ToolGateway-specific configuration.
